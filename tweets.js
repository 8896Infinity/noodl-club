/* ============================================================================
 * #NOODL post feed — curated.
 *
 * WHY CURATED, NOT AUTO-PULLED:
 *   1. X has no free public API for hashtag search; the paid API needs a secret
 *      key, which cannot live in a static site without leaking it.
 *   2. The #NOODL hashtag is not ours alone — X's own "top" results for #NOODL
 *      are mostly 2019-2020 Japanese posts about "Noodl", an unrelated visual
 *      programming tool. Auto-pulling would put those on the front page.
 *
 * TO ADD A POST: copy a block below, fill it in from the post on X, save, and
 * push. Ordering is computed from `likes`/`retweets`/`replies` — see heat()
 * in tweets-ui.js; nothing here needs to be kept in order by hand.
 * `date` is the post's own timestamp, ISO-8601 UTC.
 * ========================================================================== */
window.NOODL_TWEETS = [
  {
    id: '2085683206247088137',
    url: 'https://x.com/NOODL_club/status/2085683206247088137',
    name: 'NOODL',
    handle: 'NOODL_club',
    date: '2026-08-07T11:03:21Z',
    text: 'Time to NOODL in!\n\n#NOODL',
    likes: 0,
    retweets: 0,
    replies: 0
  },
  {
    id: '2084967939678355624',
    url: 'https://x.com/NOODL_club/status/2084967939678355624',
    name: 'NOODL',
    handle: 'NOODL_club',
    date: '2026-08-05T11:41:08Z',
    text: '$NOODL is the first token on ODIN v2.\nThe community is already here, and NOODL.club has been built by the community.\nNOODL = New Odin HODL.\n\nFirst token.\nBest community.\nNative culture of the new Odin era.\n\nWe’re not waiting for what’s next.\nWe’re building it.\n\n#NOODL',
    likes: 7,
    retweets: 1,
    replies: 0
  },
  {
    id: '2083949197745860715',
    url: 'https://x.com/jixiofng450094/status/2083949197745860715',
    name: '清野',
    handle: 'jixiofng450094',
    date: '2026-08-02T16:13:01Z',
    text: '$NOODL · Noodle Nova\n\nODIN·FUN v2 重启后的第一枚代币。\n\n它不喊口号，也不画大饼，只说一句：No promises, just NOODL。\n\n这是一枚很“温馨”的比特币 Runes meme。名字拆开是 New Odin HODL——西方的长期持有文化，配上东方“面”的长寿寓意。面既是面条，也是体面、情面和场面。一碗面，慢慢吃到 Nova。\n\n作为平台重启后的首发代币，它天然占了位置。v1 曾经热闹过，也翻过车；v2 在安全上做了明显升级，TSS 托管、多签治理，团队把钥匙分得更开。$NOODL 踩在这个新起点上，叙事干净，没有历史包袱。\n\n目前市值还在早期，持有者不多，社区也在慢慢长。官网 noodl.club 写得很用心，把文化、平台逻辑和长期视角都摊开讲了。它不是那种靠喊麦起飞的币，更像一碗慢慢煮的面——有味道，也经得起等。\n\n比特币一直缺一个真正有辨识度的 meme。$NOODL 尝试用“第一”和“长期主义”去填这个位置。\n\n感兴趣的，自己去看看。\n\n@Odin_GodOfRunes @NOODL_club\n\n#NOODL #ODINFUN #Bitcoin #Runes #NoodleNova',
    likes: 0,
    retweets: 1,
    replies: 3
  },
  {
    id: '2083919000782967133',
    url: 'https://x.com/SuZhe_eth/status/2083919000782967133',
    name: 'SuZhe',
    handle: 'SuZhe_eth',
    date: '2026-08-02T14:13:01Z',
    text: '人生三碗面：场面、情面和体面。\n\nnoodl.club 社区网站，蓄势待发\n\n#noodl',
    likes: 13,
    retweets: 3,
    replies: 0
  },
  {
    id: '2079951746538037336',
    url: 'https://x.com/jixiofng450094/status/2079951746538037336',
    name: '清野',
    handle: 'jixiofng450094',
    date: '2026-07-22T15:28:34Z',
    text: '#NOODL #ODINFUN$NOODL — the pioneer meme on the reborn Bitcoin platform.Lightning-fast trading, strong community, still micro cap.Early birds, let’s build the NOODL Universe!',
    likes: 0,
    retweets: 0,
    replies: 0
  },
  {
    id: '2079568242142752791',
    url: 'https://x.com/NOODL_club/status/2079568242142752791',
    name: 'NOODL',
    handle: 'NOODL_club',
    date: '2026-07-21T14:04:40Z',
    text: 'The First NOODL Festival has come to an end!\n\nThank you to everyone who shared your NOODL stories and joined the journey.\n\nTogether, we distributed 17,000 $NOODL to the community.\n\nThe more $NOODL you hold, the bigger your reward from this event.\n\nBut this is just the beginning.\n\nThe next NOODL airdrop is coming soon…\n\nKeep building.\nKeep believing.\nKeep NOODLing.\n\nodin.fun/token/hjrg\n\n#NOODL #ODINFUN',
    likes: 3,
    retweets: 0,
    replies: 1
  },
  {
    id: '2077782146748703149',
    url: 'https://x.com/SuZhe_eth/status/2077782146748703149',
    name: 'SuZhe',
    handle: 'SuZhe_eth',
    date: '2026-07-16T15:47:21Z',
    text: 'We are just getting started.\nWe must make ODIN.FUN fun again.\n\n#NOODL @NOODL_club',
    likes: 11,
    retweets: 7,
    replies: 2
  },
  {
    id: '2077658481273037296',
    url: 'https://x.com/NOODL_club/status/2077658481273037296',
    name: 'NOODL',
    handle: 'NOODL_club',
    date: '2026-07-16T07:35:57Z',
    text: 'The First NOODL Festival is here!\n\nShare your NOODL story with us.\n\nWhether it’s:\nyour dinner tonight\nthe rebirth of ODIN·FUN\nyour thoughts on NOODL & HODL culture\n\nPost it on X, tag @NOODL_club and add #NOODL.\nSelected posts will receive $NOODL airdrops\nEvery post mentioning NOODL will also be collected on the website: NOODL.club\n\nLittle Easter egg: click the chopsticks on the website to discover the latest NOODL posts.\n\nLet’s build the NOODL universe together.\n\n#NOODL #ODINFUN',
    likes: 6,
    retweets: 2,
    replies: 0
  },
  {
    id: '2076902066681745754',
    url: 'https://x.com/SuZhe_eth/status/2076902066681745754',
    name: 'SuZhe',
    handle: 'SuZhe_eth',
    date: '2026-07-14T05:30:14Z',
    text: '第一个币，往往有 First is First 的叙事价值。\n比特币是区块链起点，加密朋克是 NFT 起点，ORDI 是铭文起点。\n对 Odin·fun 来说，NOODL 就是这个生态的起点。\n而且 NOODL = New Odin HODL，本身也代表长期持有的文化。\n另外，上一个版本最大的遗憾，是链上玩家和链下玩家形成了不同共识；这一次 V2 重启，早期共识重新汇聚一处。\n还有些原因，可以明晚 21:00 Space 聊',
    likes: 2,
    retweets: 0,
    replies: 0
  },
  {
    id: '2076676930255147265',
    url: 'https://x.com/SuZhe_eth/status/2076676930255147265',
    name: 'SuZhe',
    handle: 'SuZhe_eth',
    date: '2026-07-13T14:35:37Z',
    text: 'New Odin HODL = NOODL\n\nODIN·FUN 上一次刚要起飞，却被安全事故打断。\n这一次重新出发，就要拿住了。\n\nNOODL IN.\n\n平台已经重启，并加入 Google Authenticator，账户安全多了一层保障。\n\n当 Robinhood 热度退潮、市场被 PVP 消耗之后，资金终究会重新寻找能够长期建设和沉淀共识的地方。\n\n就像去年川普发币耗尽 SOL Pump.fun 生态后，资金重新回流比特币生态一样：热点会退潮，真正有承接能力的平台会再次被发现。\n\n目前比特币生态中，能在产品体验和交易体感上承接外部资金的平台并不多，ODIN·FUN 仍可能是一个重要入口。\n\nNOODL 作为 V2 开局的第一枚代币，\nFirst 叙事、文化属性、重启寓意，一次拉满。\n\nODIN·FUN 从来不只是一个技术平台，更是比特币生态里少见的信仰平台。\nV1 的起飞被意外打断，V2 不是简单重启，而是一次信仰的再次集结。\n\nNew Odin. New HODL.\n\nodin.fun/token/hjrg',
    likes: 10,
    retweets: 1,
    replies: 5
  },
  {
    id: '2077450893545689151',
    url: 'https://x.com/NOODL_club/status/2077450893545689151',
    name: 'NOODL',
    handle: 'NOODL_club',
    date: '2026-07-15T17:51:04Z',
    text: 'Welcome to the best club in the galaxy.\n\nnoodl.club\n\nNOODL in!\n\n#NOODL',
    likes: 2,
    retweets: 0,
    replies: 0
  },
  {
    id: '2077064246001795285',
    url: 'https://x.com/SuZhe_eth/status/2077064246001795285',
    name: 'SuZhe',
    handle: 'SuZhe_eth',
    date: '2026-07-15T00:14:00Z',
    text: '录了个 ODIN·FUN V2 新版的操作视频（以欧意 web3 手机钱包为例）@okx\n\n点此直通平台首发代币 NOODL：\nodin.fun/token/hjrg\n\n#NOODL',
    likes: 11,
    retweets: 4,
    replies: 1
  },
  {
    id: '2076874066179584492',
    url: 'https://x.com/TycheTDOG/status/2076874066179584492',
    name: 'U.mi',
    handle: 'TycheTDOG',
    date: '2026-07-14T03:38:58Z',
    text: '今天看了看奥丁平台，odin.fun，目前也就 first 概念可以玩一下，但是目前大家还是怀疑资金安全问题，这个希望能尽快解决，可以先小资金几百几千刀布局一下这个代币，#NOODL，后面解决了安全问题在考虑进大资金，至于奥丁狗还是先观望一下',
    likes: 4,
    retweets: 2,
    replies: 5
  },
  {
    id: '2076343852785697108',
    url: 'https://x.com/jincong/status/2076343852785697108',
    name: '金聪',
    handle: 'jincong',
    date: '2026-07-12T16:32:05Z',
    text: '现在加入 ODIN·FUN V2 版本，以闪电速度交易符文资产。\nodin.fun\n\n新平台第一个部署的代币是 Noodle Nova $NOODL\n\nHey @BobBodily is it the first token on ODIN·FUN?\n\n#NOODL',
    likes: 3,
    retweets: 0,
    replies: 1
  }
];
